...
import FlightDetailCards from '../Flights/flightDetailsCard';
...
function HotelSearchResults(props) {
  ...
  const { history, hotelSearchResults, returnFlightSearchResults } = props;
  // useEffect(() => {}, []);

  if (hotelSearchResults === null || hotelSearchResults.Quotes.length === 0)
    history.push('/flightSearch');
  else {
    const destination = hotelSearchResults.Places[0].Name;
    const origin = hotelSearchResults.Places[1].Name;
        ...
        <section>
          {hotelSearchResults !== null ? (
            <div>
              {' '}
              <h5>DEPARTURES</h5>
              {hotelSearchResults.Quotes.map((result) => {
                return (
                  <FlightDetailCards
                    id={result.QuoteId}
          ...
          {returnFlightSearchResults !== null ? (
            <div>
              ...
              {returnFlightSearchResults.Quotes.map((result) => {
                return (
                  <FlightDetailCards
                    id={result.QuoteId + hotelSearchResults.Quotes.length}
                    setToggleButtonDisplay={setToggleButtonDisplay}
                    tripType={'return'}
